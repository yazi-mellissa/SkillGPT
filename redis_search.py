...
class RedisMemory:
    ...
    def init_esco_embeddings(self) -> None:
        ...
        def make_json_serializable(data):
            """Convert any non-serializable types within the data dictionary to serializable ones."""
        ...
        for esco_index in REDIS_ESCO_INDICES:            
            ...
            df_data = pd.concat((pd.read_parquet(f"{ESCO_EMBEDDINGS_DIR}/df_{esco_index}_{language}.parquet") for language in ["fr"]))            
            ...
            for batch in chunk(zip(range(vec_num), zip(data_records, esco_embeddings)), 10000):
                ...
                for key, (data, embedding) in batch.items():    
                    ...
                    pipe.hset(f"{esco_index}:{key}", mapping={b"data": json.dumps(data_json), "embedding": embedding.astype(np.float32).tobytes()})                    
                pipe.execute()
